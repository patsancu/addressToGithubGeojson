...
class Address_to_gist_map():
    ...
    def get_coordinate_set_from_strings(self):
        coordinate_set = map(self.from_string_to_coordinates_and_poi, self.addresses)
        if self.debug:
            print coordinate_set
        self.fromCoordinatesToFeatureCollection(coordinate_set)
        if self.debug:
            print self.fc

    def from_string_to_coordinates_and_poi(self, address_string):
    ...
    def get_icon_name_for_poi_type(self, poi_type):
        if poi_type == "bar":
            return "bar"
        print "Poi_type is not a bar, it's a: {}".format(poi_type)
        return 'triangle'

    def fromCoordinatesToFeatureCollection(self, coordinatesSet, address_string = None):
        feature_collection = []
        for coordinate in coordinatesSet:
            ...
            properties = {}
            if poi_type:
                properties['marker-symbol'] = self.get_icon_name_for_poi_type(poi_type)
    ...
    def create_string_from_geojson(self):
        dump = geojson.dumps(self.fc, sort_keys=True)
        return dump

    ### Writes string to file
    def writeContentsToFile(self, contents, file_name):
        if write_to_file_flag:
            write_to_file(filename, dump)

    def createGistDictFromGeojson(self, filename, description):
        content = self.create_string_from_geojson()
        public = True
        gist_data = {}
    ...
    def gist_map_from_addresses(self, filename="some_map.geojson", description="some description"):
        self.get_coordinate_set_from_strings()
        self.createGistDictFromGeojson(filename, description)
        response, gist_url = self.publishToGists()
        print gist_url
